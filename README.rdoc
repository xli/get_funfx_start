...
Try out steps:
  1. I tested it with Flex Builder SDKS 3.2 on Mac currently, for other SDKS version / OS, you should set FB_SDK_PATH env variable to point to Flex SDK path (e.g.: FB_SDK_PATH=PATH_TO_AIR_SDK/sdks/3.2.0).
  2. cucumber & funfx
  3. using safariwatir, for I can't find out simple solution for FireWatir to solve Firefox one profile per instance problem on CI build.
